...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances,
               controller_feats, generouter_feats=None):
        """
        ...
        instances.controller_feats = cat([
            # Reshape: (N, -1, Hi, Wi) -> (N*Hi*Wi, -1)
            x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in controller_feats
        ], dim=0,)
        instances.generouter_feats = cat([
            # Reshape: (N, -1, Hi, Wi) -> (N*Hi*Wi, -1)
            x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in generouter_feats
        ], dim=0,)

        return self.fcos_losses(instances)

    def fcos_losses(self, instances):
        results, losses, extras = {}, {}, {}
        ...
        # full_controller_feats = instances.controller_feats
        full_locations = instances.locations
        ...
        # extras["full_controller_feats"] = full_controller_feats
        extras["full_locations"] = full_locations

        return results, losses, extras

    def predict_proposals(
            self, logits_pred, reg_pred, ctrness_pred,
            locations, image_sizes, controller_feats, generouter_feats=None, gt_instances=None
    ):
        if self.training:
        ...
        # gt_inds = self._get_ground_truth(locations, gt_instances)["target_inds"]

        sampled_boxes = []

        # bundle = {"l": locations, "o": logits_pred, "r": reg_pred, "c": ctrness_pred, "s": self.strides,
        #           "gt_inds": gt_inds, "ctrl": controller_feats, "gnot": generouter_feats}
        bundle = {"l": locations, "o": logits_pred, "r": reg_pred, "c": ctrness_pred, "s": self.strides,
                  "ctrl": controller_feats, "gnot": generouter_feats}

        # full_controller_feats = cat([x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in controller_feats], dim=0)
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            c = per_bundle["c"]
            ctrl = per_bundle["ctrl"]
            gnot = per_bundle["gnot"]
            # gt_inds = per_bundle["gt_inds"]

            # sampled_boxes.append(
            #     self.forward_for_single_feature_map(
            #         l, o, r, c, image_sizes, ctrl, gnot, gt_inds
            #     )
            # )
            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c, image_sizes, ctrl, gnot
                )
            )
        ...
        # res = {"instances": boxlists, "full_controller_feats": full_controller_feats}
        res = {"instances": boxlists}

        return res

    def forward_for_single_feature_map(
            self, locations, logits_pred, reg_pred,
            ctrness_pred, image_sizes, ctrl, gnot=None
    ):
        ...
        ctrl = ctrl.view(N, -1, H, W).permute(0, 2, 3, 1)
        ctrl = ctrl.reshape(N, H * W, -1)
        gnot = gnot.view(N, -1, H, W).permute(0, 2, 3, 1)
        gnot = gnot.reshape(N, H * W, -1)
        ...
        for i in range(N):
            ...
            # per_gt_inds = gt_inds[per_box_loc]

            per_ctrl = ctrl[i]
            per_ctrl = per_ctrl[per_box_loc]
            per_gnot = gnot[i]
            per_gnot = per_gnot[per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                # per_gt_inds = per_gt_inds[top_k_indices]

                per_ctrl = per_ctrl[top_k_indices]
                per_gnot = per_gnot[top_k_indices]
            ...
            # boxlist.gt_inds = per_gt_inds
            boxlist.controller_feats = per_ctrl
            boxlist.generouter_feats = per_gnot
